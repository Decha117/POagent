...
- **OCR**:
  - `OCR_MODE=typhoon`: Typhoon OCR 1.5 2B inference via Transformers แบบ local-only (`local_files_only=True`) โดย `TYPHOON_MODEL_PATH` ต้องชี้ไปที่โฟลเดอร์โมเดลในเครื่อง พร้อม fast fallback เมื่อเกิดข้อผิดพลาด.
